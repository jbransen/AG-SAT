...
\newcommand{\agdeplineupupctrl}[5]{ \draw[->,style=agdepstyle] (#2.north) .. controls +(0,#5) and +(0,#5) .. (#3.north); }
...
\newcommand{\agdeplinedowndownctrl}[5]{ \draw[->,style=agdepstyle] (#2.south) .. controls +(0,-#5) and +(0,-#5) .. (#3.south); }
...
\newcommand{\agvarlinedownup}[6]{ \draw[<->,style=agdepstyle] (#2.south) .. controls +(0,-1) and +(0,1) .. node[#5] {#6} (#3.north); }
\newcommand{\agvarlinedowndown}[6]{ \draw[->,style=agidpstyle] (#2.south) .. controls +(0,-1) and +(0,-1) .. node[#5] {#6} (#3.south); }
\newcommand{\agvarlineupup}[6]{ \draw[->,style=agidpstyle] (#2.north) .. controls +(0,1) and +(0,1) .. node[#5] {#6} (#3.north); }
\newcommand{\agpicture}[1]{ \begin{tikzpicture}\input{#1.tikz}\end{tikzpicture} }

\newcommand{\exprrelsize}{0.20cm}
\newcommand{\exprdist}{0.20cm}
\tikzstyle{litnode}=[circle,draw,minimum size=\exprrelsize]
\tikzstyle{astline}=[]
\tikzstyle{synnode}=[rectangle,draw,black,minimum size=\exprrelsize]
\tikzstyle{inhnode}=[rectangle,draw,black,minimum size=\exprrelsize]
\tikzstyle{synline}=[gray,->, thick]
\tikzstyle{chord}=[black,<->, thick]
\tikzstyle{inhline}=[gray,->, thick]
...
\newcommand{\mydoublepic}[5]{
 \begin{figure}
  \centering
    \begin{minipage}[a]{#2\textwidth}
        \agpicture{diagrams/#1}
    \end{minipage}
    \begin{minipage}[a]{#4\textwidth}
        \agpicture{diagrams/#3}
    \end{minipage}
    \caption{#5}
  \label{fig:#1}
 \end{figure}
